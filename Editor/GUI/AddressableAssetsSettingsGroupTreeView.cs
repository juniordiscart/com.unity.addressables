...
namespace UnityEditor.AddressableAssets.GUI
{
	...
	internal class AddressableAssetEntryTreeView : TreeView
	{
        ...
        private AddressableAssetEntryIconLazyLoad m_lazyLoader = new AddressableAssetEntryIconLazyLoad();
        ...
        private void LazyLoadIcons(IList<TreeViewItem> rows, bool recurseGroups = false)
        {
            ...
            {
                if (recurseGroups || (row is AssetEntryTreeViewItem assetEntryItem && assetEntryItem.entry != null))
                    LazyLoadIcons(row);
            }
        }
        ...
        protected override void SingleClickedItem(int id)
        {
            ...
            {
                var item = FindItemInVisibleRows(id);
                if (item is AssetEntryTreeViewItem assetEntryItem && assetEntryItem.entry != null)
                {
    ...
    internal class AddressableAssetEntryIconLazyLoad
    {
        ...
        private readonly Queue<AssetEntryTreeViewItem> m_needsIconRefresh = new Queue<AssetEntryTreeViewItem>();
        private readonly Queue<AssetEntryTreeViewItem> m_needsIconRefreshPriority = new Queue<AssetEntryTreeViewItem>();
        private readonly List<AssetEntryTreeViewItem> m_reloadWhenCacheInvalidated = new List<AssetEntryTreeViewItem>();
        ...
        // Texture Caches
        private static Texture s_prefabIcon;
        private readonly Dictionary<string, Texture> m_scriptGuidToIconCache = new Dictionary<string, Texture>();
        ...
        internal void RemoveLazyIconLoadCallback()
        {
            ...
            if (m_editorProgressId > 0)
                ...
                UnityEditor.Progress.Finish(m_editorProgressId);
#endif
        ...
        private void WorkOnIconQueue()
        {
            ...
            {
                AssetEntryTreeViewItem item = null;
                // Always try and pull from priority queue first
                if (m_needsIconRefreshPriority.Count > 0)
                {
                    item = m_needsIconRefreshPriority.Dequeue();
                }
                else if (m_needsIconRefresh.Count > 0)
                {
                    item = m_needsIconRefresh.Dequeue();
                }
                else
                {
