...
namespace UnityEditor.AddressableAssets.Build.DataBuilders
{
    ...
    public class BuildScriptPackedMode : BuildScriptBase
    {
        ...
        internal bool CreateCatalogFiles(byte[] data, AddressablesDataBuilderInput builderInput, AddressableAssetsBuildContext aaContext, string catalogHash, ContentCatalogBuildInfo buildInfo)
        {
            ...
            WriteFile(buildPath.Replace(".bin", ".hash"), HashingMethods.Calculate(data).ToString(), builderInput.Registry);
            ...
            if (aaContext.Settings.BuildRemoteCatalog)
            {
                if (buildInfo != null)
                {
                    dependencyHashes = CreateRemoteCatalog(data, aaContext.runtimeData.CatalogLocations, aaContext.Settings, builderInput, new ProviderLoadRequestOptions()
                    {
                        IgnoreFailures = true,
                        WebRequestTimeout = aaContext.Settings.CatalogRequestsTimeout
                    }, catalogHash);
                }
                else
                {
        ...
        internal bool CreateCatalogFiles(string jsonText, AddressablesDataBuilderInput builderInput, AddressableAssetsBuildContext aaContext, string catalogHash, ContentCatalogBuildInfo buildInfo)
        {
            ...
            else
            {
                ...
                WriteFile(buildPath.Replace(".json", ".hash"), HashingMethods.Calculate(jsonText).ToString(), builderInput.Registry);
            }
            ...
            if (aaContext.Settings.BuildRemoteCatalog)
            {
                if (buildInfo != null)
                {
                    dependencyHashes = CreateRemoteCatalog(jsonText, aaContext.runtimeData.CatalogLocations, aaContext.Settings, builderInput, new ProviderLoadRequestOptions() {
                        IgnoreFailures = true,
                        WebRequestTimeout = aaContext.Settings.CatalogRequestsTimeout
                    }, catalogHash);
                }
